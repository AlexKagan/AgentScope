...
|---|---|---|---|
| Host environment leakage into the sandbox | Empty-baseline sandbox env + positive allowlist (`build_sandbox_environment`) | Non-allowlisted names dropped/rejected; module never reads `os.environ`; `sbx exec` always invoked with explicit `NAME=value`, never a bare `-e NAME` (which was confirmed to copy from the *local* AgentScope process's own environment) | Phase 0 unit (`test_environment_policy.py`); Phase 1A.1 real-backend black-box (`tests/external/test_sbx_environment_isolation.py`) |
...
| Prompt-injected exfiltration via network | Sandbox-scoped `--deny-network "**"` at creation (ADR-less, `docs/findings/sbx-cli.md`) — does not depend on the host machine's own global network policy | No credentials placed in the sandbox environment (see above); tool policy + approvals remain a later-phase concern | Phase 1A.1 real-backend black-box (`tests/external/test_sbx_network_isolation.py`) |
| Runaway/malicious process outlasting its command timeout | `execute()` issues `sbx stop` on timeout and invalidates the runtime (ADR 0011) — killing only the local CLI process does not stop the remote command | Runtime moves to a non-reusable state after any timeout, forcing a fresh sandbox for further work | Phase 1A.1 real-backend black-box (`tests/external/test_sbx_timeout.py`), replicating the Step 1 spike's heartbeat methodology |
